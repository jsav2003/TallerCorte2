"""
MÃ³dulo que implementa el patrÃ³n Adapter para conversiÃ³n de unidades de medida.
"""
...
class ConversorLongitud(IConversorUnidades):
    ...
    # Factores de conversiÃ³n a metros como unidad base
    FACTORES_A_METROS = {
...
class ConversorArea(IConversorUnidades):
    """Conversor para unidades de Ã¡rea."""

    # Factores de conversiÃ³n a metros cuadrados como unidad base
    FACTORES_A_METROS2 = {
        UnidadMedida.CENTIMETROS: 0.0001,  # cmÂ²
        UnidadMedida.METROS: 1.0,          # mÂ²
        UnidadMedida.PULGADAS: 0.00064516, # inÂ²
        UnidadMedida.PIES: 0.092903        # ftÂ²
    }

    def convertir(self, valor, unidad_origen, unidad_destino):
        """Convierte un valor de Ã¡rea entre unidades."""
        if unidad_origen == unidad_destino:
            return valor
...
class ConversorVolumen(IConversorUnidades):
    ...
    # Factores de conversiÃ³n a metros cÃºbicos como unidad base
    FACTORES_A_METROS3 = {
        UnidadMedida.CENTIMETROS: 0.000001,    # cmÂ³
        UnidadMedida.METROS: 1.0,              # mÂ³
        UnidadMedida.PULGADAS: 0.000016387,    # inÂ³
        UnidadMedida.PIES: 0.028317            # ftÂ³
    }
...
class UnidadAdapter:
    """
    Adapter que facilita la conversiÃ³n de medidas de figuras geomÃ©tricas.
    Implementa el patrÃ³n estructural Adapter.
    """
    ...
    def convertir_medidas_figura(self, figura, unidad_origen, unidad_destino):
        ...
        if unidad_origen not in UnidadMedida.get_unidades_disponibles():
            raise ValueError(f"Unidad de origen no vÃ¡lida: {unidad_origen}")

        if unidad_destino not in UnidadMedida.get_unidades_disponibles():
            raise ValueError(f"Unidad de destino no vÃ¡lida: {unidad_destino}")
    ...
    def validar_unidad(unidad):
        ...
        if unidad not in UnidadMedida.get_unidades_disponibles():
            ...
            raise ValueError(f"Unidad no vÃ¡lida: {unidad}. Unidades disponibles: {unidades}")
        return True
